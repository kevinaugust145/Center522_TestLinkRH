...
@interface MFi_DeviceListController () <ISControlManagerDeviceList> {
    ...
    UIButton *stop;
    UIButton *refresh;
    UUIDSettingViewController *uuidSetting;
    UIView *footer;
}
...
- (void)setUUID:(id)sender {
    if (!uuidSetting) {
        uuidSetting = [[UUIDSettingViewController alloc] initWithNibName:@"UUIDSettingViewController" bundle:nil];
    }
    [self.navigationController pushViewController:uuidSetting animated:YES];
}
    ...
    for (ISDataPath *d in devices) {
        if ([d isKindOfClass:[ISBLEDataPath class]]) {
            if ([connectList indexOfObject:d] == NSNotFound) {
                //Added by Meet
                ISBLEDataPath *dataPath = d;
                NSLog(@"dataPath.advName %@", dataPath.advName);
                NSLog(@"dataPath.UUID %@", dataPath.UUID);
                if ([dataPath.advName containsString:@"BT"]) {
...
//    ISBLEDataPath *dataPath = [deviceList objectAtIndex:0];
//    NSLog(@"dataPath.advName = %@", dataPath.advName);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath {
    switch (indexPath.section) {
        case 0:
            ...
            break;
        case 1: {
            ISDataPath *dataPath = [deviceList objectAtIndex:indexPath.row];
